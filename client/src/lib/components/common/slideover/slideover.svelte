    ...
    <div class="fixed inset-0 overflow-hidden">
      <div class="absolute inset-0 overflow-hidden">
        <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
          <div
            class="min-w-screen bg-base-100 pointer-events-auto w-screen max-w-md"
            transition:fly={{ x: '100%', opacity: 0.5, duration: 500 }}>
            <div
              class="border-base-200 flex h-full flex-col overflow-y-scroll border shadow-xl">
              <div class="bg-primary px-4 py-6 sm:px-6">
                <div class="flex items-center justify-between">
                  <h2 class="text-primary-content pr-4 text-xl font-semibold leading-6">
                    {title}
                  </h2>
                ...
                <div class="mt-1 pr-4">
                  <p class="text-primary-content text-sm text-opacity-80">{subtitle}</p>
                </div>
              </div>
