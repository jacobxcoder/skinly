<script lang="ts">
  ...
  // If you don't want to use any of the social media links,
  // you can remove them from this array
  const socialMediaLinks: Record<string, string> = {
    ...
    twitter: 'https://twitter.com/jacob_the_coder',
    github: 'https://github.com/jacobxcoder',
    youtube: 'https://youtube.com/your-youtube-profile'
  } as const;
</script>
  ...
  <div class="mx-auto max-w-7xl overflow-hidden px-6 py-20 sm:py-24 lg:px-8">
    <nav class="-mb-6 columns-2 sm:flex sm:justify-center sm:space-x-12">
      {#each links as { name, href }}
        <div class="pb-6">
          <a {href} class="text-sm leading-6">{name}</a>
        </div>
      {/each}
    </nav>

    <div class="mt-10 flex justify-center space-x-10">
      {#each Object.keys(socialMediaLinks) as sm}
        <a
          href={socialMediaLinks[sm]}
          target="_blank"
          class="text-base-content-secondary">
          {#if sm === 'facebook'}
